...
describe('Sales Service', () => {
  describe('1. Insert a new sale on the database', () => {
    describe('when it`s a invalid payload', () => {
      before(async () => {
        sinon.stub(SalesProductsService, 'updateProductQuantity').resolves({});
        sinon.stub(SalesModel, 'createSale').resolves(1);
      });
      after(async () => {
        SalesProductsService.updateProductQuantity.restore();
        SalesModel.createSale.restore();
      });
      it('returns an object', async () => {
        const response = await SalesService.createSale(invalidPayload);
        expect(response).to.be.an('object');
      });
      ...
      it('the object contains a status code and a error message', async () => {
        ...
        expect(response.code).to.be.equal(400);
        expect(response).to.have.property('message');
        expect(Object.values(response.message)[0]).to.be.equal('"product_id" is required');
      });
    });
    describe('when it`s a valid payload', () => {
      before(async () => {
        sinon.stub(SalesProductsService, 'updateProductQuantity').resolves(null);
        sinon.stub(SalesModel, 'createSale').resolves(1);
      });
      after(async () => {
        SalesProductsService.updateProductQuantity.restore();
      ...
      it('returns an object', async () => {
        const response = await SalesService.createSale(validPayload)
      ...
      it('the object contains a status code and a sucess message', async () => {
        ...
        expect(response.message).to.have.property("id");
        expect(response.message.id).to.be.equal(1);
        expect(response.message).to.have.property("itemsSold");
        expect(response.message.itemsSold).to.have.length(2);
        expect(response.message.itemsSold[0]).to.have.property("quantity");
      });
    });
  });
  describe('2. Request a product By it`s id from the database', () => {
